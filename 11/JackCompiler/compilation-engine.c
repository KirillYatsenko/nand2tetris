...
struct compilation_engine *compilation_engine_init(const char *file)
{
	...
	engine->writer = vm_writer_init(file);
	if (!engine->writer) {
		free(engine);
		return NULL;
	}

	engine->tokenizer = tokenizer_init(file);
	if (!engine->tokenizer) {
		vm_writer_deinit(engine->writer);
		free(engine);
		return NULL;
	}

	engine->class_symbols = symbols_init();
	if (!engine->class_symbols) {
		vm_writer_deinit(engine->writer);
	...
	engine->sub_symbols = symbols_init();
	if (!engine->sub_symbols) {
		vm_writer_deinit(engine->writer);
		tokenizer_deinit(engine->tokenizer);
		symbols_deinit(engine->class_symbols);
		free(engine);
		return NULL;
	}
